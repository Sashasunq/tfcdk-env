...
export class M2pLoadbalancerProductionStack extends Construct {
    constructor(
        scope: Construct,
        id: string,
        ...
        // bookkeeper lb
        const bookkeeperLb = new Lb(scope, `elb-m2pslots-bookkeeper-103`, {
            name: 'elb-m2pslots-bookkeeper-103',
            internal: true,
        ...
        new LbListener(scope, `elb-m2pslosts-staging-103-listener`, {
            ...
            defaultAction: [
                {
                    type: 'forward',
                    targetGroupArn: m2pBase.bookkeeperTargetProduction.arn,
                },
            ],
        });

        // slots lb
        const lb = new Lb(scope, `elb-m2pslots-103`, {
            name: 'elb-m2pslots-103',
            internal: false,
            loadBalancerType: 'application',
            securityGroups: [
        ...
        const listenerHttp = new LbListener(scope, `elb-m2pslots-103-listener-http`, {
        ...
        const listenerHttps = new LbListener(scope, `elb-m2pslots-103-listener-https`, {
            loadBalancerArn: lb.arn,
            port: 443,
            protocol: 'HTTPS',
            defaultAction: [
        ...
        for (const m2pSlotConfig of m2pSlotConfigs) {
            ...
            new LbListenerRule(scope, `elb-m2pslots-103-http-rule-${m2pSlotConfig.name}`, {
                ...rule,
                listenerArn: listenerHttp.arn,
            });

            new LbListenerRule(scope, `elb-m2pslots-103-https-rule-${m2pSlotConfig.name}`, {
                ...rule,
